...
const { Schema, arrayOf, EntityStore, FilteredList, Reference } = require('../src');
...
describe('Resources', () => {
  describe('update(schema, data)', () => {
    it('stores data normalized', () => {
      ...
      store = store.for(SearchResults, state => state.setItems(filter, items));

      expect(store.getResource(SearchResults).getItems(filter)).to.eql([
        {
          id: 1,
          title: 'Foo',
          host: REF(User, 2),
